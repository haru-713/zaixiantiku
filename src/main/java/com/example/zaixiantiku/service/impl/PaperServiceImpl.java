    ...
    public PaperVO autoGeneratePaper(PaperAutoGenerateDTO generateDTO) {
        ...
        for (AutoGenerateRuleDTO.TypeDistributionDTO dist : rule.getTypeDistribution()) {
            ...
            if (available.size() < dist.getCount()) {
                QuestionType questionType = questionTypeMapper.selectById(dist.getTypeId());
                String typeName = questionType != null ? questionType.getTypeName() : String.valueOf(dist.getTypeId());
                throw new RuntimeException("题库中题型 [" + typeName + "] 数量不足，需要 " + dist.getCount() + " 题，现有 "
                        + available.size() + " 题");
            }
